...
//        }
//    }
//    HAL_UART_Receive(&huart4, buffer, 1, 1000);
//    if (buffer[0] == "$") {
//        uint16_t numBytes = 0;
//        HAL_UARTEx_ReceiveToIdle(&huart4, buffer + 1, 1023, &numBytes, HAL_MAX_DELAY);
//        HAL_StatusTypeDef status = HAL_UARTEx_ReceiveToIdle(&huart4, (uint8_t*)buffer + 1, NMEA_BUFFER_SIZE - 2, &numBytes, 100);
